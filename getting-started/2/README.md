...
We had earlier used `set` to create new nodes. Although, on using the `uid` of an
...
Similarly, you can add new predicates to an existing node.
Since the predicate `country` doesn't exist for the node with the uid `0x13881`,
...
## Adding an edge between existing nodes
You can add an edge between existing nodes using their uids.

Let's say that `Mary` starts to follow `Michael`.
...
Traversal queries start their operation from a set of root nodes. These root nodes
are selected using Dgraph's built-in functions. Then, they follow the edges from
these root nodes and return the nodes at the other end of the edges.
...
`Michael` only follows one person, hence, the traversal returns just one node.
...
that makes multi-level deep queries or traversals easy?
